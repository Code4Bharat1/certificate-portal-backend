...
const drawMJPunctuality = async (
  ...
  // Bullet points
  const bullet1 = `• Your performance evaluation, appraisal, and growth opportunities may be adversely impacted, and`;
  currentY = wrapText(ctx, bullet1, leftMargin, currentY, contentWidth, 19);
  currentY += 15;

  const bullet2 = `• Continued non-compliance may result in disciplinary action, including formal warnings or other corrective measures, as per Company policy.`;
  ...
  // Closing
  const paragraph4 = `This letter serves as a formal warning, and no further reminders will be issued. Continued violation of punctuality and discipline norms will lead to strict enforcement of disciplinary actions without exception.`;
  currentY = wrapText(ctx, paragraph4, leftMargin, currentY, contentWidth, 19);
  currentY += 20;
...
export default drawMJPunctuality;
